...
Menu::Menu(sf::RenderWindow& window, const sf::Font* fontOpt)
: m_window(window),
  m_visible(true),
  m_ownedFont(),
  m_font(nullptr),
  ...
  m_selected(0)
{
    ...
    if (fontOpt) {
        ...
        bool ok = m_ownedFont.openFromFile("assets/Pokemon_GB.ttf");
        if (!ok) {
...
void Menu::layout() {
    ...
    // Reposicionar input nombre si fuera necesario
    m_cajaNombre.setPosition({m_window.getSize().x * 0.5f, m_window.getSize().y * 0.6f});
...
void Menu::activateCurrent() {
    if (m_selected == NuevaPartida) {
        ...
        if (onCargarPartida) onCargarPartida();
    }
}
...
void Menu::manejarInputNombre(const sf::Event& ev) {
    if (ev.is<sf::Event::TextEntered>()) {
        ...
        if (u == '\r' || u == '\n') {
            if (!m_nombreJugador.empty() && onNuevaPartida) {
                onNuevaPartida(m_nombreJugador);
            }
            m_pidiendoNombre = false;
            m_title.setString("Menu Principal");
            layout();
...
void Menu::handleEvent(const sf::Event& ev) {
    ...
    if (m_pidiendoNombre) {
        manejarInputNombre(ev);
        return;
    }

    if (ev.is<sf::Event::KeyPressed>()) {
        const auto& key = ev.getIf<sf::Event::KeyPressed>();
        if (key->scancode == sf::Keyboard::Scan::Up)   selectNext(-1);
        if (key->scancode == sf::Keyboard::Scan::Down) selectNext(+1);
        if (key->scancode == sf::Keyboard::Scan::Enter) activateCurrent();
...
void Menu::draw() {
    ...
    if (m_pidiendoNombre) {
        m_window.draw(m_cajaNombre);
        m_window.draw(m_textoNombre);
    } else {
        for (int i = 0; i < NUM_OPCIONES; ++i) {
